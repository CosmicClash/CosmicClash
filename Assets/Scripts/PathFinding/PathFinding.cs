...
public class PathFinding : MonoBehaviour {
	public static class Grid{
		...
		public static void Build(GridInfo info){
			...
			for(int y = 0;y<Grid.sizeY;y++){
				for(int x = 0;x<Grid.sizeX;x++){
					...
					if(x>0&&y>0){
						tempNode.addConectionsDiag(Nodes[index-Grid.sizeX-1]);
					}
					if(x<Grid.sizeX-1&&y>0){
						tempNode.addConectionsDiag(Nodes[index-Grid.sizeX+1]);
					}
				}
			}
		...
		public static void test(){
			...
			for(int x = 0;x<Nodes.Count;x++){
				...
				Nodes[x].scriptNode.nodesDiags=Nodes[x].neighborDiag;
		...
		public static Result buildPath(int startX,int startY,int endX,int endY){
			...
			while (true){
				count++;
				//Debug.Log("Checking Furthest Node: "+count+": "+openList.Count+"-"+workingNode.x+", "+workingNode.y);
	...
	public class Node{
		...
		public IList<Node> neighborDiag = new List<Node>();
		...
		public void addConectionsDiag(Node newNode){
			this.addSingleConnectionDiag(newNode);
			newNode.addSingleConnectionDiag(this);
		}
		public void addSingleConnectionDiag(Node newNode){
			this.neighborDiag.Add(newNode);
		}
		#if test
		public void updateEnabled(){
			...
			this.scriptNode.walkable=this.enabled;
		}
		#endif
		public void getHValue(int x,int y){
			this.h=(Mathf.Abs(this.x-x)+Mathf.Abs(this.y-y)*50);
		...
		public void checkNeighbors(){
			for( int x = 0;x<this.neighbor.Count;x++){
				...
				if(current.parent==null||current.steps>this.steps+10){
					current.nearestSegment(this);
					Grid.addToOpenList(current);
					continue;
				}
			}
			for( int x = 0;x<this.neighborDiag.Count;x++){
				Node current = this.neighborDiag[x];
				if(current.enabled==false){
					continue;
				}
				if(current.parent==null||current.steps>this.steps+14){
					current.nearestSegment(this);
					Grid.addToOpenList(current);
					continue;
				}
